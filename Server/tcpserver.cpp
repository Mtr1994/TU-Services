...
void TcpServer::incomingConnection(qintptr socketDescriptor) //多线程必须在此函数里捕获新连接
{
    ...
    connect(tcpTemp,&TcpSocket::sgl_close_client,this, &TcpServer::slot_close_client, Qt::QueuedConnection);//NOTE:断开连接的处理，从列表移除，并释放断开的Tcpsocket，此槽必须实现，线程管理计数也是考的他
    connect(tcpTemp, &TcpSocket::sgl_read_data, this, &TcpServer::sgl_read_data, Qt::QueuedConnection); // 数据信号
    connect(this, &TcpServer::sentDisConnect ,tcpTemp, &TcpSocket::disConTcp, Qt::QueuedConnection);//断开信号
    connect(this, &TcpServer::sgl_sent_data ,tcpTemp, &TcpSocket::slot_sent_data, Qt::QueuedConnection);//断开信号

    tcpTemp->moveToThread(th);//把tcp类移动到新的线程，从线程管理类中获取
    tcpClient->insert(socketDescriptor, tcpTemp);//插入到连接信息中

    QString key = QString("%1:%2").arg(serverAddress().toString()).arg(serverPort());
    emit sgl_new_client(key, socketDescriptor, ip, port);
}

void TcpServer::slot_close_client(int handle,const QString & ip, quint16 prot,QThread * th)
{
    tcpClient->remove(handle);//连接管理中移除断开连接的socket
    ThreadHandle::getClass().removeThread(th); //告诉线程管理类那个线程里的连接断开了
    QString key = QString("%1:%2").arg(serverAddress().toString()).arg(serverPort());
    emit sgl_close_client(key, handle, ip, prot);
}
